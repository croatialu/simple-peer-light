...
export const getConfig = thunky((cb) => {
  ...
  get.concat('https://instant.io/__rtcConfig__', (err, res, data) => {
    if (err)
      return cb(err)
    data = data.toString()
    try {
...
let canvas
export const getMediaStream = function () {
  if (exports.wrtc) {
...
export const isBrowser = function (name) {
